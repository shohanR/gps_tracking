...
var map = null;
var selectedDeviceId = null;
var deviceMarkers = {}; // Store markers for all devices
  ...
  devicesRef.on('value', function (snapshot) {
    ...
    snapshot.forEach(function (childSnapshot) {
      ...
      row.addEventListener('click', function () {
        ...
        // Update the map markers for the selected device
        selectedDeviceId = device_id;
        updateMapMarkers();
      });

      // If a device was previously selected, highlight it
      if (selectedDeviceId === device_id) {
        row.classList.add('highlighted');
      }

      // Store device marker
      createOrUpdateDeviceMarker(device_id, deviceData.lat, deviceData.lng);
    });
...
// Function to create or update a device marker
function createOrUpdateDeviceMarker(deviceId, lat, lon) {
  if (!deviceMarkers[deviceId]) {
    deviceMarkers[deviceId] = L.layerGroup(); // Create a layer group for the device
    deviceMarkers[deviceId].addTo(map); // Add the layer group to the map
  }

  var marker = L.marker([lat, lon]);
  deviceMarkers[deviceId].addLayer(marker);
}

// Function to update the map markers for the selected device
function updateMapMarkers() {
  // Hide all markers first
  for (var deviceId in deviceMarkers) {
    map.removeLayer(deviceMarkers[deviceId]);
  }

  if (selectedDeviceId && deviceMarkers[selectedDeviceId]) {
    // Show markers for the selected device
    map.addLayer(deviceMarkers[selectedDeviceId]);
  }
}

function logout() {
  // Clear the selected device ID and markers on logout
  selectedDeviceId = null;
  for (var deviceId in deviceMarkers) {
    map.removeLayer(deviceMarkers[deviceId]);
  }
